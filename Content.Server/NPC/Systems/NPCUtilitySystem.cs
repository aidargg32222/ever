...
>>>>>>> upstream/master
    ...
    private void Add(NPCBlackboard blackboard, HashSet<EntityUid> entities, UtilityQuery query)
    {
        ...
        switch (query)
        {
            ...
            case PlayerHostilesQuery:
                {
                    var queryMinds = _entityManager.EntityQueryEnumerator<MindContainerComponent>();
                    while (queryMinds.MoveNext(out var ent, out var _))
                    {
                        if (_mindSystem.TryGetMind(ent, out var _, out var mindId) && !_mindSystem.IsCharacterDeadPhysically(mindId))
                        {
                            entities.Add(ent);
                        }
                    }
